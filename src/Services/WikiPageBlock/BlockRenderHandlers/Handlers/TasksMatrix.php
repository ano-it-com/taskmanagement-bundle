...
class TasksMatrix implements HandlerInterface
{
    ...
    public function handle(WikiPageBlock $block): ?WikiPageBlock
    {
        ...
        foreach ($matches[1] as $key => $match) {
            ...
            //try {
            /** @var  $dataObject */
            $dataObject = $this->em->getRepository(DataObject::class)->findByTitleAndType($stageName, $this->trackerType)[0];

            $tasks               = $this->treeService->getTasksByStage($dataObject->getId());
            $allowedProjectNames = $this->treeService->getAllowedProjectNames();

            $newHtml = $this->makeTable($tasks, $dataObject->getId(), $allowedProjectNames);
    ...
    protected function makeTable($tasks, $trackerId, $allowedProjectNames)
    {
        ...
        $tracker = $this->em->getRepository(DataObject::class)->find($trackerId);
...
//        $table = $this->makeFilter($allowedProjectNames, $filteredProjectName);
        ...
        foreach ($tasks as $project) {
            ...
            $table .= sprintf(
                '<tr 
                            data-matrix-region-id="%s" 
                            data-matrix-region-name="%s" 
                            data-matrix-person-id="%s" 
                            data-matrix-person-name="%s" 
                            data-matrix-district-id="%s" 
                        >',
        ...
        $table .= sprintf('<br><a class="btn btn-primary" href="%s">Экспорт в Excel</a>', $this->makeLinkForDownload($tracker->getTitle()));

        return $table;
    }


    public function makeTableForExcel($tasks, $trackerId, $allowedProjectNames)
    {
        /** @var DataObject[] $stages */
        $stages = $this->em->getRepository(DataObject::class)->findBy([
            'type' => $this->stageType
        ...
        foreach ($stages as $stage) {
            $tableHeadRow[] = $stage->getTitle();
        }

        $tableData[] = $tableHeadRow;

        $tableDataTotalCountsRow   = [ 'Итог' ];
        $tableDataTotalCountsRow[] = $totalPercents;

        foreach ($stages as $stage) {
            if (array_key_exists($stage->getTitle(), $stageCounts)) {
                $tableDataTotalCountsRow[] =
                    $this->getPercentsString($stageCounts[$stage->getTitle()]['done_tasks'], $stageCounts[$stage->getTitle()]['total_tasks']);
            }
        }

        $tableData[] = $tableDataTotalCountsRow;
        ...
        foreach ($tasks as $project) {
            ...
            $taskDataRow[] = $project['title'];

            $taskDataRow[] = $percentOfDone . '%';
            ...
            foreach ($stages as $stage) {
                ...
                foreach ($project['children'] as $projectStage) {
                    if ($projectStage['title'] === $stage->getTitle()) {
                        $count = $this->getPercentsString($projectStage['done_tasks'], $projectStage['total_tasks']);
                        break;
                    }
                }
    ...
    protected function makeLinkForDownload($stageName)
    {
        return sprintf('/wiki/dashboard/matrix/download?stage=%s', $stageName);
    }
